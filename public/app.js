...
const TRANSLATIONS = {
  ka: {
    argon2Loading:    "🔐 Argon2id — 3-5 წამი...",
    keysLoading:      "🔑 გასაღებები...",
    mlkemLoading:     "🔮 ML-KEM-1024 გასაღებები...",
    mlkemShort:       "🔮 ML-KEM-1024...",
    drActive:         "🔐 PQXDH + Double Ratchet: აქტიური",
    drWaiting:        "⏳ მეორე მომხმარებელს ელოდება...",
    fpTitle:          "შეადარე მეგობარს — MITM შეუძლებელია",
    errRequired:      "ოთახის კოდი და პაროლი საჭიროა.",
    errTooLong:       "ძალიან გრძელი.",
    loading:          "[ იტვირთება... ]",
    joinBtn:          "შესვლა",
    errGeneral:       "შეცდომა. სცადეთ თავიდან.",
    errArgon2:        "❌ Argon2 ვერ ჩაიტვირთა. გვერდი გადაიტვირთეთ (F5).",
    errKyber:         "❌ Kyber ვერ ჩაიტვირთა. გვერდი გადაიტვირთეთ (F5).",
    notJoined:        "⏳ მეორე მომხმარებელი ჯერ არ შემოუერთდა.",
    rekey:            "🔄 Session PFS: Re-key — დატოვე ოთახი და ხელახლა შემოდი ახალი გასაღებებით.",
    sent:             "[ გაგზავნილი ]",
    deleting:         "წაიშლება...",
    copy:             "კოპირება",
    secSuffix:        "წმ",
  ...
  en: {
    argon2Loading:    "🔐 Argon2id — 3-5 sec...",
    keysLoading:      "🔑 Keys...",
    mlkemLoading:     "🔮 ML-KEM-1024 keys...",
    mlkemShort:       "🔮 ML-KEM-1024...",
    drActive:         "🔐 PQXDH + Double Ratchet: Active",
    drWaiting:        "⏳ Waiting for second user...",
    fpTitle:          "Compare with friend — MITM impossible",
    errRequired:      "Room code and password required.",
    errTooLong:       "Too long.",
    loading:          "[ Loading... ]",
    joinBtn:          "Join",
    errGeneral:       "Error. Please try again.",
    errArgon2:        "❌ Argon2 failed to load. Refresh page (F5).",
    errKyber:         "❌ Kyber failed to load. Refresh page (F5).",
    notJoined:        "⏳ Second user hasn't joined yet.",
    rekey:            "🔄 Session PFS: Re-key — Leave room and rejoin with new keys.",
    sent:             "[ Sent ]",
    deleting:         "Deleting...",
    copy:             "Copy",
    secSuffix:        "s",
...
function t(key) {
  const lang = localStorage.getItem("encchat_lang") || "ka";
  return (TRANSLATIONS[lang] || TRANSLATIONS.ka)[key] || key;
}
...
//         ML-KEM-1024 (Kyber)       → კვანტური KEM
...
//
//  კვანტური კომპიუტერი ვერ გატეხს — ML-KEM-1024 NIST 2024 სტანდარტი
...
async function deriveSK_Argon2id(password, roomId) {
  ...
  showJoinStatus(t("argon2Loading"));
...
// ════════════════════════════════════════════════════════════════════
//  ML-KEM-1024 (CRYSTALS-Kyber) — Post-Quantum KEM
...
async function joinRoom() {
  ...
  if (!room || !pass)          { showError(t("errRequired")); return; }
  if (room.length > 128 || pass.length > 256) { showError(t("errTooLong")); return; }
  ...
  document.getElementById("join-btn").textContent = t("loading");
  ...
  try {
    ...
    // 2. X25519 keypair
    showJoinStatus(t("keysLoading"));
    _myX25519    = await generateX25519();
    _myX25519B64 = await exportX25519Pub(_myX25519);

    // 3. ML-KEM-1024 keypair
    showJoinStatus(t("mlkemLoading"));
...
async function sendMessage() {
  ...
  if (!text) return;
  if (!dr || !dr.ready) { showChatNotice(t("notJoined")); return; }
...
async function renderMessages(records) {
  ...
  for (const rec of records) {
    ...
    let text;
    if (mine) {
      text = sentCache.get(rec.id) || t("sent");
    } else {
...
function formatTime(secs) {
  if (secs <= 0) return t("deleting");
  if (secs < 60) return `${secs}${t("secSuffix")}`;
