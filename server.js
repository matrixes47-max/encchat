 ...
 * mlkemPub — base64 ML-KEM-1024 public key (1568 bytes)
 * pqct — base64 ML-KEM-1024 ciphertext (1568 bytes)
 */
app.post("/api/keys", rateLimiter(CONFIG.RATE_LIMIT_MAX_REQUESTS), (req, res) => {
  ...
  if (mlkemPub && !isValidBase64(mlkemPub, 2300))
    return res.status(400).json({ error: "invalid mlkemPub" });
  if (pqct && !isValidBase64(pqct, 2300))
