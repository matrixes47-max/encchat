...
function runEsbuild(args) {
  ...
  const cmd = process.platform === "win32"
    ? `npx esbuild ${joined}`
    : `${quote(path.join(__dirname, "node_modules", ".bin", "esbuild"))} ${joined}`;
...
`);
  ...
  //
  // app.js expects:
  //   Kyber1024.KeyGen()       → { publicKey, privateKey }
  //   Kyber1024.Encrypt(pk)    → { cipherText, sharedSecret }
  //   Kyber1024.Decrypt(sk,ct) → sharedSecret (Uint8Array)
  //
